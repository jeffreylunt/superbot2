...
- **Canary health checks:** a message starting with `[canary <id>]` is an automated end-to-end check from the telegram watcher, NOT from Jeff. Reply with exactly `[canary-ack <id>]` via `SendMessage({to: "dashboard-user"})` and do NOTHING else — no logging, no escalation, no analysis, minimal tokens. The watcher swallows the ack; Jeff never sees canary traffic.
